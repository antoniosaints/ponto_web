<template>
  <div class="p-4">
    ...
    <span href="#" class="flex justify-between mb-4 p-4 bg-gray-300 rounded-lg shadow dark:bg-gray-700">
      <div class="flex flex-col">
        <h5 id="relogio_ponto" class="mb-2 text-4xl dark:text-white text-gray-900 font-bold tracking-tight">
          {{ horario }}
        </h5>
        <p class="font-normal text-gray-800 dark:text-gray-300">Horário atual</p>
      </div>
      <div class="flex flex-col justify-center text-end">
        <h5 id="relogio_ponto" class="text-2xl dark:text-gray-300 text-gray-900 font-bold tracking-tight">
          {{ dataMes }}
        </h5>
    ...
    <div class="d-flex text-right p-0">
      ...
      <button @click="verifyLocation" type="button"
      ...
      <button @click="centerUserLocation" type="button"
        ...
        <iconComponent color="text-white" icon="location-crosshairs" size="text-l" />
      </button>
    </div>
  </div>
</template>
...
const cercasUserAutorized = ref([
  {
    latlon: "-4.029875152665384, -44.46592515807999",
    raio: 100,
    nome: "Cerca 1",
  }
]);
...
async function initializeComponents(lat, lon) {
  const cercas = await storeLocales.getLocations();
  cercasUserAutorized.value = cercas.data;
...
function createCircle(latlon, raio, description = "", color) {
...
async function verifyLocation() {
  ...
  if (isLocationAutorized.find((item) => item.isCheck === true)) {
    ...
    toast.success(`Ponto registrado no local de trabalho: ${valor.nome}`, "Feito");
  } else {
    toast.warning("Você não está dentro de um local de trabalho cadastrado!", "Atenção");
  }
}
